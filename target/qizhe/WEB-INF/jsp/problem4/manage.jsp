...
<div id="page-content" class="clearfix">
  ...
  <div class="row-fluid">
	...
	<div class="row-fluid">
			...
			<!-- 检索  -->
			<form action="adIssue4/list.do" method="post" name="Form" id="Form">
			<table>
				<tr>
					<td>
			...
			<table id="table_report" class="table table-striped table-bordered table-hover">
				...
				<thead>
					<tr>
						...
						<th class="center">加油站名称</th>
						<th class="center">问卷得分率</th>
						<th class="center">申诉情况</th>
						<th class="center">视频下载</th>
						<th class="center">申诉开关</th>
					...
					<c:when test="${not empty varList}">
						...
						<c:forEach items="${varList}" var="var" varStatus="vs">
							<tr>
								...
								<td class='center' ><a href="<%=basePath%>adIssue4/goEdit.do?ID=${var.ID}">	${var.qname}</a></td>
								<td class='center'>${var.qscore}</td>
								<td class='center'>${var.beiyong3}</td>
								<td class='center'><a href="<%=basePath%>uploadFiles/file/${var.qgongsi }-${var.qname}.zip">视频或者图片下载</a></td>
	...
	<!-- PAGE CONTENT ENDS HERE -->
		...
		function add(){
			 ...
			 diag.Drag=true;
			 diag.Title ="新增";
			 diag.URL = '<%=basePath%>adIssue4/goAdd.do';
            diag.Width = 1960;
            diag.Height = 1020;
		...
		function del(Id){
			bootbox.confirm("确定要删除吗?", function(result) {
				if(result) {
					top.jzts();
					var url = "<%=basePath%>adIssue4/delete.do?ID="+Id+"&tm="+new Date().getTime();
		...
		function edit(Id){
			 ...
			 diag.Drag=true;
			 diag.Title ="编辑";
			 diag.URL = '<%=basePath%>adIssue4/goEdit.do?ID='+Id;
			 diag.Width = 1960;
			 diag.Height = 1020;
			 diag.CancelEvent = function(){ //关闭事件
                 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
				...
				if(result) {
					...
					if(str==''){
						...
						if(msg == '确定要删除选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: '<%=basePath%>adIssue4/deleteAll.do?tm='+new Date().getTime(),
						    	data: {DATA_IDS:str},
								dataType:'json',
